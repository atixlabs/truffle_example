...
const ContractState = props => {
  return (
    <div>
      ...
      <div className="pure-g">
        <div className="pure-u-1-1"><b>Stored Value:</b> {props.storedValue.toString()}</div>
        <div className="pure-u-1-1"><b>Call Count:</b> {props.callCount.toString()}</div>
...
class App extends Component {
  ...
  async getContractData(simpleStorageInstance) {
    const storedValue = await simpleStorageInstance.get();
    const callCount = await simpleStorageInstance.getCallCount();
  ...
  updateVal(val) {
    ...
    this.state.contract
      .set(val, {from: this.state.sender})
      .then(() => {
    ...
    return (
      <div className="App">
        <nav className="navbar pure-menu pure-menu-horizontal">
            <a href="#" className="pure-menu-heading pure-menu-link">SimpleStorage.sol</a>
        </nav>
        <main className="container">
          <div className="pure-g">
            <div className="pure-u-1-1">
              <h1>Let's do some coding!</h1>
              <p>This web app interacts with a contract named SimpleStorage.sol:</p>
              <ul>
              ...
              <ContractUpdate contract={contract} onValidInput={val => this.updateVal.bind(this)(val)} disabled={this.state.sendingTx} />
              <ContractState storedValue={contractData.storedValue} callCount={contractData.callCount} logs={logs} />
            </div>
          </div>
        </main>
      </div>
